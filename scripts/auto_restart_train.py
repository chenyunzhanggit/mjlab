...
Usage:
    python scripts/auto_restart_train.py <task_id> [training_args...]

Example:
    python scripts/auto_restart_train.py Mjlab-Tracking-Teleoperation-Unitree-G1 \\
...
class TrainingMonitor:
    ...
    def find_latest_checkpoint(self) -> Optional[tuple[Path, str]]:
        ...
        print(f"[MONITOR] No checkpoints found in any run directory")
        return None
    ...
    def build_restart_command(self, checkpoint_info: Optional[tuple[Path, str]]) -> list[str]:
        ...
        if checkpoint_info is not None:
            ...
            for i, arg in enumerate(cmd):
                ...
                if arg in ["--agent.resume", "--agent.load-run", "--agent.load-checkpoint"]:
                    skip_next = True
                    continue
                if arg.startswith("--agent.resume=") or \
                   arg.startswith("--agent.load-run=") or \
                   arg.startswith("--agent.load-checkpoint="):
                    continue
            ...
            print(f"[MONITOR] Starting fresh training (no checkpoint found)")
    ...
    def run_training(self) -> int:
        ...
        print(f"[MONITOR] Starting training process...")
    ...
    def monitor(self):
        ...
        print("=" * 80)
        print(f"[MONITOR] Auto-restart training monitor started")
...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print("Usage: python scripts/auto_restart_train.py <task_id> [training_args...]")
        print("\nExample:")
        print("  python scripts/auto_restart_train.py Mjlab-Tracking-Teleoperation-Unitree-G1 \\")
    ...
    monitor = TrainingMonitor(
        ...
        max_restarts=100,  # Adjust as needed
        restart_delay=5.0,  # Wait 5 seconds before restart
    )
